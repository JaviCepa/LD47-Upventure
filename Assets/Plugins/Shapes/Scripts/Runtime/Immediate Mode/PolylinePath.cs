...
namespace Shapes {
	...
	public class PolylinePath : IDisposable {
		...
		bool hasSetFirstPoint;

		// used to detect if mesh needs to update on the fly on draw
		bool hasMesh = false;
		...
		void TryUpdateMesh( bool closed, PolylineJoins joins ) {
			...
			ShapesMeshGen.GenPolylineMesh( mesh, path, closed, joins, true );
		}
