...
namespace PlatformerPro
{
	...
	public class AirMovement_Variable : AirMovement
	{
		...
		override protected void MoveInY ()
		{
			...
			{
				float multipliedActualMaxJumpHeight = GetJumpHeightOrForce(actualMaxJumpHeight);
				jumpHeldTimer -= TimeManager.FrameTime;
				float expectedJumpHeight = Mathf.Lerp (minJumpHeight, multipliedActualMaxJumpHeight, (maxJumpAccelerationTime - jumpHeldTimer) / maxJumpAccelerationTime);
		...
		{
			float jumpVelocity = GetJumpHeightOrForce(initialVelocity);
			jumpStart = true;
		...
		override public void DoOverridenJump(float newHeight, int ignoredJumpCount, bool skipPowerUps = false)
		{
			...
			float jumpVelocity = skipPowerUps ? newVelocity : GetJumpHeightOrForce(newVelocity);
			jumpStart = true;
			character.SetVelocityY(jumpVelocity);
			jumpHeldTimer = maxJumpAccelerationTime;
			startingYPosition = character.Transform.position.y;
