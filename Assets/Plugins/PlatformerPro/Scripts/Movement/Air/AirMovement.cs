...
namespace PlatformerPro
{
	...
	public class AirMovement : BaseMovement <AirMovement>
	{
		...
		virtual public float GetJumpHeightOrForce(float baseHeight)
		{
			...
			if (character.EquipmentManager != null && !skipEquipmentMultipliers)
			{
				multiplier *= character.EquipmentManager.TotalJumpHeightMultiplier;
			}
			// Apply upgrades
			if (character.ItemManager != null && !skipUpgradeMultipliers)
			{
				multiplier *= character.ItemManager.TotalJumpHeightMultiplier;
			}
			// Apply power-ups
			if (character.PowerUpManager != null && !skipPowerUpMultipliers)
			{
				multiplier *= character.PowerUpManager.TotalJumpHeightMultiplier;
			}
		...
		/// </summary>
		/// <returns>The acceleration.</returns>
		/// <param name="baseHeight">Base double jump height.</param>
