...
namespace PlatformerPro
{
	...
	public class AirMovement_Physics : AirMovement, IPhysicsMovement
	{
		...
		override protected void MoveInX (float horizontalAxis, int horizontalAxisDigital, ButtonState runButton)
		{
			...
			{
				// Apply acceleration
				if (Mathf.Abs (horizontalAxisDigital * actualAcceleration) > ignoreForce)
				{
					character.AddVelocity((useAnalogueInput ? horizontalAxis : (float)horizontalAxisDigital) * actualAcceleration * TimeManager.FrameTime, 0, true);
				}
		...
		{
			float actualJumpVelocity = GetJumpHeightOrForce(jumpVelocity);
			float actualDoubleJumpVelocity = GetJumpHeightOrForce(doubleJumpVelocity);
			jumpStart = true;
			jumpCount++;
			...
			{
				Vector2 velocity = new Vector2(character.Velocity.x, actualDoubleJumpVelocity);
				character.SetVelocityX(velocity.x);
				character.SetVelocityY(velocity.y);
			}
			else 
			{
				Vector2 velocity = new Vector2(character.Velocity.x, actualJumpVelocity);
		...
		override public void DoOverridenJump(float newHeight, int jumpCount, bool skipPowerUps = false)
		{
			...
			float actualVelocity = skipPowerUps ? initialVelocity : GetJumpHeightOrForce(initialVelocity);
			jumpStart = true;
			this.jumpCount = jumpCount;
			
			Vector2 velocity = new Vector2(character.Velocity.x, actualVelocity);
			if (jumpAway) velocity = Quaternion.Euler(0, 0, character.transform.rotation.eulerAngles.z) * velocity;
			character.SetVelocityX(velocity.x);
			character.SetVelocityY(velocity.y);
