...
namespace PlatformerPro
{
	...
	public class AirMovement_Digital : AirMovement, IFlippableGravityMovement
	{
		...
		/// </summary>
		protected const int MovementVariableCount = 10;
		...
		{
			float actualSpeed = GetSpeed(airSpeed);
			if (horizontalAxisDigital == 1)
			{
				character.SetVelocityX(character.IsGravityFlipped ? -actualSpeed : actualSpeed);
				character.Translate((character.IsGravityFlipped ? -actualSpeed : actualSpeed) * TimeManager.FrameTime, 0, false);
			}
