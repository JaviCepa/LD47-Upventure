...
namespace PlatformerPro
{
	...
	public class GroundMovement_LandWithDamage : GroundMovement_SimpleLand
	{
		...
		/// </summary>
		protected const int MinSpeedForDamageIndex = 3;

		/// <summary>
		/// The index of the constant damage.
		/// </summary>
		protected const int ConstantDamageIndex = 4;
		
		/// <summary>
		/// The index of the damage amount.
		/// </summary>
		protected const int DamageAmountIndex = 5;

		/// <summary>
		/// The size of the movement variable array.
		/// </summary>
		private const int TotalMovementVariableCount = 6;
		...
		override public Movement Init(Character character, MovementVariable[] movementData)
		{
            ...
            if (movementData != null && movementData.Length >= TotalMovementVariableCount)
			{
			...
			if (characterHealth == null) Debug.LogWarning ("Land with damage requires a CharacterHealth component to be on the character");
		...
		{
			if (movementData != null && movementData.Length == TotalMovementVariableCount - 1)
			{
				MovementVariable[] tmp = movementData;
				movementData = new MovementVariable[TotalMovementVariableCount];
				System.Array.Copy(tmp, movementData, GroundMovement_SimpleLand.MovementVariableCount - 1);
				System.Array.Copy(tmp, GroundMovement_SimpleLand.MovementVariableCount - 1, movementData, GroundMovement_SimpleLand.MovementVariableCount, TotalMovementVariableCount - GroundMovement_SimpleLand.MovementVariableCount + 1);
			}
			if (movementData == null || movementData.Length < TotalMovementVariableCount)
			{
				movementData = new MovementVariable[TotalMovementVariableCount];
			...
			if (movementData[MinSpeedForLandIndex] == null) movementData[MinSpeedForLandIndex] = new MovementVariable(DefaultMinSpeedForLand);
			float minSpeedForLand = movementData[MinSpeedForLandIndex].FloatValue;
