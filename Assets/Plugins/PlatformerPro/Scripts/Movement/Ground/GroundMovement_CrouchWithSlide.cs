...
namespace PlatformerPro
{
	...
	public class GroundMovement_CrouchWithSlide : GroundMovement_Crouch
	{
		...
		/// </summary>
		protected const int MinimumSpeedForSlideIndex = 11;

		/// <summary>
		/// The index of the max speed.
		/// </summary>
		protected const int MaxSpeedIndex = 12;

		/// <summary>
		/// The size of the movement variable array.
		/// </summary>
		new protected const int MovementVariableCount = 13;

		/// <summary>
		/// Offset for where we will place the standard physics data in the movement data.
		/// </summary>
		private const int PhysicsMovementDataIndexOffset = 6;
		...
		new public static MovementVariable[] DrawInspector(MovementVariable[] movementData, ref bool showDetails, Character target)
		{
			...
			{
				Debug.LogWarning("Upgrading Crouch With Slide movement data. Double check new values.");
				MovementVariable[] tmp = movementData;
				movementData = new MovementVariable[MovementVariableCount];
				System.Array.Copy(tmp, movementData, tmp.Length);
				for (int i = MovementVariableCount - 1; i > CrouchToggleIndex; i--)
				{
					movementData[i] = movementData[i - 1];
				}
				movementData[CrouchToggleIndex] = new MovementVariable(false);
			}
