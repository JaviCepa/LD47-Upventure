...
namespace PlatformerPro
{
	...
	public class GroundMovement_CrouchNoCrawl : GroundMovement_Crouch
	{
		...
		/// </summary>
		protected const int CrouchStartMustBeDownIndex = 6;
		
		/// <summary>
		/// The size of the movement variable array.
		/// </summary>
		new protected const int MovementVariableCount = 7;
		...
		override public bool CheckInput()
		{
			...
			if (actionButton == -1 && character.Input.VerticalAxisDigital == -1 && (!crouchStartMustBeDown || character.Input.HorizontalAxisDigital == 0)) return true;
			if (actionButton > -1 && character.Input.GetActionButtonState (actionButton) == ButtonState.HELD) return true;
			return false;
		}
		...
		new public static MovementVariable[] DrawInspector(MovementVariable[] movementData, ref bool showDetails, Character target)
		{
			...
			{
				Debug.LogWarning("Upgrading Crouch No Crawl movement data. Double check new values.");
				MovementVariable[] tmp = movementData;
				movementData = new MovementVariable[MovementVariableCount];
				System.Array.Copy(tmp, movementData, tmp.Length);
				movementData[CrouchStartMustBeDownIndex] = movementData[CrouchStartMustBeDownIndex - 1];
				movementData[GroundMovement_Crouch.CrouchToggleIndex] = new MovementVariable(false);
			}
