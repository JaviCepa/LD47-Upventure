	...
	public abstract class GenericResponder : PlatformerProMonoBehaviour
	{
		...
		virtual protected void DoImmediateAction(EventResponse action, System.EventArgs args)
		{
			...
			case EventResponseType.ENABLE_BEHAVIOUR:
				...
				else if (action.targetComponent is Collider) ((Collider)action.targetComponent).enabled = true;
				else if (action.targetComponent is Collider2D) ((Collider2D)action.targetComponent).enabled = true;
				break;
			case EventResponseType.DISABLE_BEHAVIOUR:
				...
				else if (action.targetComponent is Collider) ((Collider)action.targetComponent).enabled = false;
				else if (action.targetComponent is Collider2D) ((Collider2D)action.targetComponent).enabled = false;
				break;
			...
			case EventResponseType.PLAY_SONG:
				...
				if (!(player == null)) player.Play (action.message);
				break;
			...
			case EventResponseType.START_EFFECT:
				...
				{
					if (action.targetGameObject != null && action.message != null && action.message != "") ((FX_Base)action.targetComponent).StartEffect((GameObject)action.targetGameObject, action.message);
			...
			case EventResponseType.PLAY_ANIMATION:
				...
				if (animation != null)
				{
					if (string.IsNullOrEmpty(action.message)) animation.Play();
					else animation.Play(action.message);
					break;
				}
			...
			case EventResponseType.ADD_LIVES:
				...
				{
					characterHealth = ((Character) action.targetComponent).GetComponentInChildren<CharacterHealth>();
				}
			...
			case EventResponseType.SPAWN_ITEM:
				if (action.targetComponent is Spawner) 
				{
					((Spawner) action.targetComponent).SpawnNow();
				}
				else if (action.targetComponent is RandomItemSpawner) 
				{
			...
			case EventResponseType.DEACTIVATE_ITEM:
                ...
                case EventResponseType.SET_CHARACTER:
                    ...
                    {
                        Debug.LogWarning("Set Character response coudldn't find Character.");
                    }
                    break;
				case EventResponseType.ACTIVATE_PLATFORM :
					if (action.targetComponent is Platform)
					{
						((Platform)action.targetComponent).Activate(character);
					}
					else
					{
						
						Debug.LogWarning("Activate platform response expected a Platform Reference as a target.");
					}
					break;
				case EventResponseType.DEACTIVATE_PLATFORM :
					if (action.targetComponent is Platform platform)
					{
						platform.Deactivate(character);
					}
					else
					{
						Debug.LogWarning("Deactivate platform response expected a Platform Reference as a target.");
					}
					break;
				case EventResponseType.SHOW_DIALOG:
					// Need to set target component to null to enable GetCharacter to work 
					EventResponse tmp = new EventResponse(action);
					tmp.targetComponent = null;
					character = GetCharacter(tmp, args);
					var targetCompoment = action.targetComponent;
					action.targetComponent = null;
					if (character != null)
					{
						if (action.targetScriptableObject is DialogGraph)
						{
							if (action.targetComponent is DialogSystem)
							{
								((DialogSystem) action.targetComponent).StartDialog(character, (DialogGraph)action.targetScriptableObject);
							}
							else if (DialogSystem.Instance != null)
							{
								DialogSystem.Instance.StartDialog(character, (DialogGraph)action.targetScriptableObject);
							}
							else
							{
								Debug.LogWarning("Show Dialog didn't specify a Dialog System and no default Dialog System was set.");
							}
						}
						else
						{
							Debug.LogWarning("Show Dialog couldn't find DialogGraph.");
						}
					}
					else
					{
						Debug.LogWarning("Show Dialog couldn't find Character.");
					}
					break;
				case EventResponseType.ENEMY_MESSAGE:
					if (action.targetComponent is EnemyAI ai)
					{
						ai.Message(action.message);
					}
					else
					{
						Debug.LogWarning("Enemy Message response expected an Enemy AI as a target.");
					}
					break;
				case EventResponseType.SWITCH_CAMERA_ZONE:
					PlatformerProStandardCamera pproCamera = FindObjectOfType<PlatformerProStandardCamera>();
					if (pproCamera == null)
					{
						Debug.LogWarning("Couldn't switch camera zone as we couldn't find an active PlatformerPro Camera");
					}
					else
					{
						character = GetCharacter(action, args);
						pproCamera.ChangeZone((CameraZone)action.targetComponent, character);
					}
					break;
			}	
		}
		...
		{
			Character character = null;
			if (action.targetComponent == null && args is CharacterEventArgs characterEventArgs)
			{
				character = characterEventArgs.Character;
			} 
			else if (action.targetComponent != null && action.targetComponent is Character component)
			{
				character = component;
			} 
			else if (args is CharacterEventArgs eventArgs)
			{
				character = eventArgs.Character;
			} 
