...
namespace PlatformerPro
{
	...
	{
		override public string Header => "The EventResponder allows you to respond to almost anything that happens in your game, and to take an action from adding health, to showing a particle effect to reloading the scene.";

		override public string DocLink => "https://jnamobile.zendesk.com/hc/en-gb/articles/209906708-Event-Responders";

		/// <summary>
		/// Listen to events from characters loaded in to the scene.
		/// </summary>
		public bool loadedCharacters;
		...
		void OnEnable()
		{
			if (loadedCharacters)
			{
				PlatformerProGameManager.Instance.CharacterLoaded += OnCharacterLoaded;
			}
			else
			{
				AddHandler(sender);
		...
		private void OnCharacterLoaded(object o, CharacterEventArgs e)
		{
			AddHandler(e.Character.gameObject);
		}
		...
		/// </summary>
		virtual protected void AddHandler(GameObject sendingGameObject)
		{
			...
			{
				sendingComponent = sendingGameObject.GetComponent(typeName);
		...
		virtual protected bool ApplyFilters(EventResponse action, System.EventArgs args)
		{
			...
			if (args is DialogEventArgs eventArgs)
			{
				if (string.IsNullOrEmpty(stringFilter)) return true;
				if (stringFilter == eventArgs.EventId) return true;
				return false;
			}
			if (args is EnemyAIEventArgs enemyEventArgs)
			{
				if (string.IsNullOrEmpty(stringFilter)) return true;
				if (stringFilter == enemyEventArgs.EventID) return true;
				return false;
			}
			...
			if (args is TriggerEventArgs)
			{
				if (intFilter == 0 ) return true;
				if (intFilter == 1 && !((TriggerEventArgs)args).SwitchState) return true;
				if (intFilter == 2 && ((TriggerEventArgs)args).SwitchState) return true;
				return false;
			}
			if (args is ExtraDamageInfoEventArgs)
			{
				if ((stringFilter == null || stringFilter == "" || stringFilter == ((ExtraDamageInfoEventArgs)args).AttackName) &&
					...
					(intFilter <= 0 || intFilter <= ((DamageInfoEventArgs)args).DamageInfo.Amount) &&
					(altIntFilter < 0 || altIntFilter >= ((DamageInfoEventArgs)args).DamageInfo.Amount)) return true;
				return false;
			}
			if (args is DamageInfoEventArgs)
			{
				if (((DamageInfoEventArgs) args).DamageInfo == null)
				{
					// If we dont have damage info we are probably hitting scenery, approximate what we know
					if (damageTypeFilter == DamageType.NONE && intFilter <= 0) return true;
					return false;
				}
				if ((intFilter <= 0 || intFilter <= ((DamageInfoEventArgs)args).DamageInfo.Amount) &&
				    (altIntFilter < 0 || altIntFilter >= ((DamageInfoEventArgs)args).DamageInfo.Amount) &&
			...
			{
				if (stringFilter == null || stringFilter == "" || stringFilter == "NONE" || stringFilter == ((ItemEventArgs)args).Type)
				{
					if (intFilter == 0) return true;
			...
			if (args is SceneEventArgs)
			{
				if ((stringFilter == null || stringFilter == "" || stringFilter == ((SceneEventArgs)args).RespawnPoint)) return true;
				return false;
			}
