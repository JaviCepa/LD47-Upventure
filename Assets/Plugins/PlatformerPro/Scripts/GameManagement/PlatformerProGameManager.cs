...
namespace PlatformerPro
{
	...
	public class PlatformerProGameManager : PlatformerProMonoBehaviour
	{
		...
		/// </summary>
		[Tooltip ("Should we save using binary files (true) or XML files (false).")]
		...
		/// </summary>
		[Tooltip ("Should we (by default) persist platformer pro object like doors, platforms, items, etc. You can override on individual objects.")]
		...
		/// </summary>
		[Tooltip("Should we persist items spawned in to the scene.")]
		public bool spawnedItemPersistence = false;
...
#endif
            if (enablePersistence) PersistableObjectManager = PersistableObjectManager.CreateNewPersistableObjectManager (saveOnAnyChange, useBinaryFormat);
			initialised = true;
		...
		virtual protected void LoadCharacter(int playerId, string characterId)
		{
			...
			if (character == null)
			{
				Debug.LogWarning("A character hasn't been defined in the GameManager defaulting to finding one in the scene, you should fix this before you go live.");
				character = FindObjectOfType<Character> ();
				if (character == null)
				{
					Debug.LogError("CRITICAL ERROR: NO characters defined in the gameManager and there are no characters in the scene.");
					return;
				}
			}
		...
		/// <param name="playerId">Player identifier.</param>
		/// <param name="characterId">Character identifier.</param>
		public static void UnSetCharacterForPlayer(int playerId)
		{
			if (playerId < 0)
			{
				Debug.LogWarning ("The playerId must be 0 or larger.");
			...
			charactersToLoad[playerId] = null;
		}
