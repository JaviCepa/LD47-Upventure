...
namespace PlatformerPro
{
	...
	public class TimeManager : PlatformerProMonoBehaviour
	{
		...
		/// Event for pause.
		/// </summary>
		public event System.EventHandler <EmptyEventArgs> GamePaused;
		...
		virtual public void OnGamePaused()
		{
			if (GamePaused != null)
			{
				GamePaused(this, EmptyEventArgs.Instance);
			}
		}
		...
		/// </summary>
		/// <param name="setTimeScale">If set to <c>true</c> then Time.timeScale will also be set to 0.</param>
		public void Pause(bool setTimeScale, bool skipPauseMenu = false)
		{
            if (PlatformerProGameManager.Instance.GamePhase == GamePhase.GAME_OVER) return;
			paused = true;
			if (pauseMenu != null && !skipPauseMenu)
			{
				pauseMenu.Pause();
			}
