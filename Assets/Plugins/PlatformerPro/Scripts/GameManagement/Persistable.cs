...
namespace PlatformerPro
{
	...
	public abstract class Persistable : PlatformerProMonoBehaviour, ICharacterReference
	{
		...
		/// </summary>
		public event System.EventHandler <EmptyEventArgs> Loaded;
		...
		virtual protected void OnLoaded()
		{
			if (Loaded != null)
			{
				Loaded(this, EmptyEventArgs.Instance);
			}
		}
		...
		public static void Save(Persistable p)
		{
			...
			if (PlatformerProGameManager.Instance.PersistableObjectManager.UseBinary)
			{
				SaveBinary(p);	
			}
			else
		...
		/// </summary>
		/// <param name="p">Persistable to save.</param>
		public static void SaveXml(Persistable p)
		{
		...
		/// </summary>
		/// <param name="p">Persistable to save.</param>
		public static void SaveBinary(Persistable p)
		{
			object saveData = p.SaveData;
			// We assume all saveData is annotated with [Serializable] we could add a condition for ISeriazable too.
			if (saveData.GetType().IsSerializable)
			{
				BinaryFormatter formatter = PlatformerProGameManager.Instance.PersistableObjectManager.BinaryFormatter;
				var memoryStream = new MemoryStream();
				using (memoryStream)
				{
					formatter.Serialize(memoryStream, saveData);
				}
				PlayerPrefs.SetString(p.PlayerPrefsIdentifier, Convert.ToBase64String(memoryStream.ToArray()));
			}
			else
			{
				Debug.LogError("Save data for " + p + " is not serializable.");
		...
		public static void Load(Persistable p)
		{
			...
			if (PlatformerProGameManager.Instance.PersistableObjectManager.UseBinary)
			{
				savedObject = LoadSavedDataBinary (p);
			}
			else
			{
				savedObject = LoadSavedDataXml (p);
			}
		...
		protected static object LoadSavedDataXml(Persistable p) {
		...
		/// <returns>The saved data.</returns>
		/// <param name="p">P.</param>
		protected static object LoadSavedDataBinary(Persistable p) {
			string data = PlayerPrefs.GetString(p.PlayerPrefsIdentifier, "");
			if (data.Length > 0)
			{
				MemoryStream memoryStream = new MemoryStream(Convert.FromBase64String(data));
				object savedObject = PlatformerProGameManager.Instance.PersistableObjectManager.BinaryFormatter.Deserialize(memoryStream);
				return savedObject;
			}
			return null;
		}
