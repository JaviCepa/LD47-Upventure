...
namespace PlatformerPro
{
	...
	public class LevelManager : Persistable
	{
		...
		/// </summary>
		public event System.EventHandler <RespawnEventArgs> Respawned;
		...
		virtual protected void OnRespawned(Character character, int playerId, string respawnPoint)
		{
			if (Respawned != null)
			{
				Respawned (this, new RespawnEventArgs (character, playerId, respawnPoint));
			}
		}
			...
			if (respawnPoint != null)
			{
				foreach (RespawnPoint r in respawnPoints)
				{
					if (r.identifier == respawnPoint)
					{
						...
						character.Respawn (r, character.LastFacedDirection);
						OnRespawned (character, character.PlayerId, r.identifier);
						return;
					}
				}
			}
