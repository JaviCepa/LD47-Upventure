...
namespace PlatformerPro
{
	...
	public class PersistableObject : PlatformerProMonoBehaviour 
	{
		...
		/// </summary>
		[HideInInspector]
		[Tooltip ("Unique ID for this object. You shouldn't need to edit this, use editor debug mode if you must.")]
		...
		/// </summary>
		[HideInInspector]
		[Tooltip ("GameObject to apply persistence settings to, defaults to self.")]
		...
		public void SetPersistenceState(bool state)
		{
			...
			{
				PlatformerProGameManager.Instance.PersistableObjectManager.SetState (guid, state, ExtraPersistenceData, spawnedObjectName, transform.position);
			}
		}
		...
		/// </summary>
		/// <param name="state">State to set.</param>
		public void SetPersistenceState(bool state, string extraPersistenceData)
		{
			if (!enablePersistence) return;
			if (string.IsNullOrEmpty(spawnedObjectName))
			{
		...
		virtual protected void ProcessState()
		{
			...
			if (data.state)
			{
				...
				case PersistableObjectType.ACTIVATE_DEACTIVATE:
					...
					ApplyExtraPersistenceData(data);
					break;
		...
		/// <summary>
		/// Apply any extra data. This differs from custom persistence, as custom persistence must control
		/// both data and ctivation state.
		/// </summary>
		/// <param name="data">Data.</param>
		virtual protected void ApplyExtraPersistenceData(PersistableObjectData data)
		{
			if (!enablePersistence) return;
			Debug.LogWarning("This persistable object doesn't support a extra persistence data.");
		}
