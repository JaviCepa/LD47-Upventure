...
namespace PlatformerPro
{
	...
	public class EnemyMovement_ShootWithDeployAndAim : EnemyMovement_SimpleShoot
	{
		...
		{
			if (state == LocalState.READY && enemy.AI != null && enemy.CurrentTarget != null)
			{
				if (enemy.CurrentTarget.transform.position.x > transform.position.x && enemy.LastFacedDirection == -1)
				{
					enemy.SwitchDirection();
				}
				else if (enemy.CurrentTarget.transform.position.x < transform.position.x && enemy.LastFacedDirection == 1)
	         	{
					enemy.SwitchDirection();
				}
			}
