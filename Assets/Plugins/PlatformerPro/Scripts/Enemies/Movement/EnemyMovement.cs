...
namespace PlatformerPro
{
	...
	/// </summary>
	public abstract class EnemyMovement : PlatformerProMonoBehaviour
	{
		#region members
...
#if UNITY_EDITOR
		/// <summary>
		/// Gets the header string used to describe the component in the inspector.
		/// </summary>
		...
		override public string Header
		{
			get
			{
				var description = ((MovementInfo)this.GetType().GetProperty("Info")?.GetValue(null, null))?.Description;
				return (string.IsNullOrEmpty(description)) ? "An enemy movement" : description;
			}
		}
#endif
		...
		/// Does the movement.
		/// </summary>
		/// <returns><c>true</c>, if this movement has been updated to match state, <c>false</c> if the movement doesn't know how to handle this state or control can't be transitioned.</returns>
