...
namespace PlatformerPro
{
	...
	public class EnemyMovement_Distributor : EnemyMovement {
		...
		void Reset()
		{
			...
			if (ai == null) return;
			EnemyState[] aiStates = ai.Info;
			statesToMovements = new EnemyStateToMovement[aiStates.Length];
			for (int i = 0; i < aiStates.Length; i++)
			{
				statesToMovements[i] = new EnemyStateToMovement();
				statesToMovements[i].state = aiStates[i];
			}
		}
		...
		void Refresh()
		{
			if (statesToMovements == null || statesToMovements.Length == 0)
			{
				Reset();
				return;
			}
			EnemyAI ai = (EnemyAI) gameObject.GetComponent (typeof(EnemyAI));
			if (ai == null) return;
			List<EnemyState> states = ai.Info.ToList();
			List<EnemyState> allStates = statesToMovements.Select(s=>s.state).ToList();
			List<EnemyStateToMovement> newStates = statesToMovements.ToList();
			for (int i = 0; i < states.Count; i++)
			{
				if (!allStates.Contains(states[i]))
				{
					EnemyStateToMovement newState = new EnemyStateToMovement {state = states[i]};
					newStates.Add(newState);
				}
			}
			statesToMovements = newStates.ToArray();
		}
#endif
	}
}