...
namespace PlatformerPro
{
	...
	public class EnemyMovement_Charge : EnemyMovement
	{
        ...
        /// </summary>
        protected Vector2 preparedOffset;
		...
		{	
			...
			{
                preparedOffset = projectileAimer.GetAimOffset(enemy);
                go.transform.position = enemy.transform.position + (Vector3)preparedOffset;
            }
			else
			{
        ...
        {
            // Update position in case character moves during animation
            if (preparedProjectile != null)
            {
                if (projectileAimer != null)
                {
                    preparedProjectile.transform.position = enemy.transform.position + (Vector3)preparedOffset;
                }
                else
                {
                    preparedProjectile.transform.position = enemy.transform.position;
                }
                preparedProjectile.Fire(damageAmount, damageType, preparedDirection, enemy);
            }
