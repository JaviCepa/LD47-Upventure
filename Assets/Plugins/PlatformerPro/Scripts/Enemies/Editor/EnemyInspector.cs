	...
	public class EnemyInspector : PersistableObjectInspector
	{
		...
		virtual protected void DrawEnemyDebugger(Enemy enemy)
		{
			...
			if (debugEnemy)
			{
				...
				DrawPrefixAndValue("Rotation", enemy.SlopeTargetRotation.ToString(), enemy.PreviousRotation.ToString());
				
				GUILayout.Space (10);
				GUILayout.Label("Target", EditorStyles.boldLabel);
				GUILayout.BeginHorizontal();
				GUILayout.Label("GameObject", EditorStyles.label);
				GUILayout.Label( enemy.CurrentTarget?.name);
				GUILayout.EndHorizontal();
				GUILayout.BeginHorizontal();
				GUILayout.Label("Range", EditorStyles.label);
				if (enemy.CurrentTarget != null) {
					GUILayout.Label("" + Vector2.Distance(enemy.CurrentTarget.transform.position,enemy.transform.position));
				}
				GUILayout.EndHorizontal();
				GUILayout.Space (10);
