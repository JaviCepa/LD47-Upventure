...
namespace PlatformerPro
{
	...
	public class PowerBombAttack : BasicAttacks
	{
		...
		override protected void StartAttack(int attackIndex)
		{
			...
			{
				Debug.LogWarning("Projectile attacks not supported for power bomb");
			}
			else if (attacks[attackIndex].useAnimationTime )
			{
				Debug.LogWarning("Animation synced attacks not supported for power bomb");
			}
			else
			{
		...
		override protected IEnumerator DoAttack(int attackIndex)
		{
			...
			{
				int damageAmount = GetAttackDamage(attacks[currentAttack]);
				DamageType damageType = attacks[currentAttack].damageType;
				...
				attacks [attackIndex].hitBox.EnableImmediate ();
			...
			if (landingDamageHitHox != null)
			{
				...
				landingDamageHitHox.EnableImmediate();
