...
namespace PlatformerPro
{
	...
	public class BasicAttacksInspector : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			DrawMultiplierSettings((BasicAttacks) target);
		...
		public static bool DrawMultiplierSettings(BasicAttacks movement)
		{
			bool changed = false;
			// Header
		...
		{
			GUILayout.Label("Attack Details for " + attack.name, EditorStyles.boldLabel);
			...
			bool useAnimationTime = EditorGUILayout.Toggle(new GUIContent("Use Animation Time", "Should we use the normalised time of the attack animation to control the time?"), attack.useAnimationTime);
			if (useAnimationTime != attack.useAnimationTime)
			{
				attack.useAnimationTime = useAnimationTime;
			}	
			if (!attack.useAnimationTime) {
			...
			string animationSpeedParam = EditorGUILayout.TextField(new GUIContent("Animation Speed Param", "If not empty this field will be set on the animator to control animation speed."), attack.animationSpeedParam);
			if (animationSpeedParam != attack.animationSpeedParam)
			{
				attack.animationSpeedParam = animationSpeedParam;
			}
			
			if (attack.attackType != AttackType.PROJECTILE)
			{
				bool useAnimationEvents = EditorGUILayout.Toggle(new GUIContent("Use Animation Events", "If true instead of enabling the hitbox from the attack we wait for animation events to be sent from the hit box."), attack.useAnimationEvents);
				if (useAnimationEvents != attack.useAnimationEvents)
				{
					attack.useAnimationEvents = useAnimationEvents;
				}
				if (!attack.useAnimationEvents)
				{
			...
			else
			{
                ...
                // Projectile delayed fire
                bool projectileDelayFire = EditorGUILayout.Toggle(new GUIContent("External Fire Event", "If true we don't actually fire the projectile but wait for an external call (used for syncing projectiles with animations)."), attack.delayFire);
                if (projectileDelayFire != attack.delayFire)
                {
                    attack.delayFire = projectileDelayFire;
                }
            }
            GUILayout.Label("Weapon", EditorStyles.boldLabel);
            string weaponSlot = EditorGUILayout.TextField(new GUIContent("Weapon Slot", "If an item is in this slot it will take durability damage each time the weapon connects. We will also get the damage type from this weapon of it is not set to NONE."), attack.weaponSlot);
            if (weaponSlot != attack.weaponSlot)
            {
                attack.weaponSlot = weaponSlot;
            }
            if (weaponSlot != null && weaponSlot != "" && weaponSlot != "NONE") { 
