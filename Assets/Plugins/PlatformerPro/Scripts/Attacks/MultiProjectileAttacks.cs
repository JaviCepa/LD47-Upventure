	...
	public class MultiProjectileAttacks : BasicAttacks
	{
		...
		virtual protected IEnumerator FireSingleProjectile(MultiProjectileAttackData attack, MultiProjectileData data)
		{
			...
			if (projectile != null) {
				...
				int damageAmount =  GetAttackDamage(attack);
				DamageType damageType = attack.damageType;
				if (!string.IsNullOrEmpty(attack.weaponSlot))
				{
					ItemInstanceData weapon = character.EquipmentManager.GetItemForSlot(attack.weaponSlot);
					if (weapon != null && weapon.Data != null && weapon.Data.damageType != DamageType.NONE) damageType = weapon.Data.damageType;
				}
				projectile.Fire (damageAmount, damageType, direction, character, Charge);
			}
