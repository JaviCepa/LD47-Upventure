...
namespace PlatformerPro
{
	...
	public class ParticleEffect_Bridge : MonoBehaviour, IAnimationBridge
	{
		...
		protected void Init()
		{
			...
			foreach (ParticleEffectMapping p in mappings)
			{
				...
				{
					if (p.state == AnimationState.NONE && p.previousState == AnimationState.NONE) Debug.LogWarning("You can't set both  state and previous state to NONE");
		...
		protected void AnimationStateChanged( object sender, AnimationEventArgs args)
		{
			...
			foreach (ParticleEffectMapping p in mappings)
			{
				...
				if ((p.state == AnimationState.NONE || p.state == args.State) && (p.previousState == AnimationState.NONE || p.previousState == args.PreviousState) && (p.groundLayer == 0 ||  ((1 << myCharacter.GroundLayer) & p.groundLayer) == (1 << myCharacter.GroundLayer)))
				{
		...
		protected void DoPlay(ParticleEffectMapping p)
		{
			switch (p.effectType)
			{
				case ParticleEffectType.ATTACHED_TO_CHARACTER:
					...
					else if (p.clearSystem) p.particleSystem.Clear();
					if (p.rotateToFacingDirection)
					{
						if (myCharacter.LastFacedDirection == -1 ) p.particleSystem.transform.eulerAngles = new Vector3(0, 0, 180.0f);	
						else p.particleSystem.transform.eulerAngles = new Vector3(0, 0, 0);
					}
					break;
				...
				case ParticleEffectType.INSTANTIATE_NEW_INSTANCE:
					...
					if (myCharacter.LastFacedDirection == -1 && p.rotateToFacingDirection) newPs.transform.Rotate(0,0, 180.0f);
					break;
	...
	public class ParticleEffectMapping
	{
		...
		/// </summary>
		[DontShowWhen("isStop", true)]
		public bool clearSystem;
		...
		/// <summary>
		/// If true the effects game object will be set to Euler 0,0,180 degrees if facing left and 0,0,0 degrees if facing right.
		/// For instantiated objects the game object will rotated by 180 degrees in Z when facing left. 
		/// </summary>
		public bool rotateToFacingDirection;
