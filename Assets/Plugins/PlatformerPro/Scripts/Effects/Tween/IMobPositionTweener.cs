...
namespace PlatformerPro.Tween
{
	...
	/// </summary>
	public class IMobPositionTweener : MonoBehaviour, ITweener <IMob, Vector2>
	{

		protected Vector2 destination;
		protected float speed;
		protected IMob target;
		protected Action<IMob, Vector2> callback;
		...
		public void TweenWithTime (TweenMode tweenMode, IMob target, Vector2 destination, float time, Action<IMob, Vector2> callback)
		{
			// Early exits
			...
			{
				float distance = Vector2.Distance (((Component)target).transform.position, destination);
				speed = distance/ time;
		...
		/// <returns>The isntance doing the tween.</returns>
		public void TweenWithRate (TweenMode tweenMode, IMob target, Vector2 destination, float speed, Action<IMob, Vector2> callback)
		{
			// Early exits
		...
		protected IEnumerator DoTween()
		{
			...
			currentPosition = (Vector2) ((Component)target).transform.position;
