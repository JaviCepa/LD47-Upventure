...
namespace PlatformerPro.Extras
{
	...
	public class UIInventorySlot : PlatformerProMonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
	{
        ...
        /// </summary>
        [Tooltip("If true then this slot acts like a button.")]
		...
		virtual protected void PostInit()
		{
			...
			{
				UIInventory[] invs = GetComponentsInParent<UIInventory> ();
                for (int i = 0; i < invs.Length; i++)
                {
                    if (!(invs[i] is UIShop || invs[i] is UIEquipmentManager || invs[i] is UIInventoryButtonGroup))
                    {
                        uiInventory = invs[i];
                    }
                }
            }
            if (uiInventory == null) Debug.LogWarning("No matching inventory type found for slot");
        }
		...
		{
            // If we don't have the invetory yet, force an update
            if (uiInventory == null) PostInit();

			myPosition = position;
			myData = data;
			if (!ShouldShowItemInUiSlot(position, data, isSelected, isPicked))
			{
        ...
        /// </summary>
        virtual protected bool ShouldShowItemInUiSlot(int position, ItemInstanceData data, bool isSelected, bool isPicked)
        {
            if (data == null || data.amount == 0) return false;
            return true;
        }
