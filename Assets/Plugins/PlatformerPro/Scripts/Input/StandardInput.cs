...
namespace PlatformerPro
{
	...
	public class StandardInput : Input
	{
		...
		void LateUpdate() 
		{
			...
			consumeJumpButton = false;
			consumeRunButton = false;
			if (consumeActionButton != null)
			{
				for (int i = 0 ; i < consumeActionButton.Length; i++)
				{
					consumeActionButton[i] = false;
				}
			}
		}
			...
			if (data.Length > 0)
			{
				try 
				{
					...
					using (StringReader stream = new StringReader(data)) {
						...
						consumeActionButton = new bool[actionButtons.Length];
						return true;
					}
				}
		...
		override public ButtonState JumpButton
		{
			get
			{
				ButtonState state = GetStateForKey(jump);
				if (consumeJumpButton && state == ButtonState.DOWN) state = ButtonState.HELD;
				return state;
			}
		}
		...
		override public ButtonState RunButton
		{
			get
			{
				ButtonState state = GetStateForKey(run);
				if (consumeRunButton && state == ButtonState.DOWN) state = ButtonState.HELD;
				return state;
			}
		}
		...
		override public ButtonState GetActionButtonState(int buttonIndex)
		{
			...
			if (buttonIndex < actionButtons.Length)
			{
				...
				ButtonState state = GetStateForKey(actionButtons[buttonIndex]);
				if (consumeActionButton[buttonIndex] && state == ButtonState.DOWN) state = ButtonState.HELD;
				return state;
			}
			return ButtonState.NONE;
		}
		...
		/// This implementation simply zeros out the buffer timer.
		/// </summary>
		override public void ConsumeJumpButton()
		{
			consumeJumpButton = true;
		}
		...
		/// This implementation simply zeros out the buffer timer.
		/// </summary>
		override public void ConsumeRunButton()
		{
			consumeRunButton = true;
		}
		...
		/// This implementation simply zeros out the buffer timer.
		/// </summary>
		override public void ConsumeActionButton(int index)
		{
			if (consumeActionButton.Length > index) consumeActionButton[index] = true;
		}
