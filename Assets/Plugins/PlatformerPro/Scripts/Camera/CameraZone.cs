...
namespace PlatformerPro
{
	...
	/// </summary>
	public class CameraZone : PlatformerProMonoBehaviour
	{
		
#if UNITY_EDITOR
		override public string Header => "An area which the camera can be moved to and within. Sets the boundaries of your levels viewable area " +
		                                 "(you must use a PlatformerPro Camera for this to apply).";
		...
		[Header("Gizmo")]
		public Color zoneColor = new Color(0,0,1,0.05f);
		public Color outlineColor = new Color(0,0,1,0.25f);
		...
		public Vector3 GetBestPositionForCharacter(Camera camera, Character character)
		{
			...
			// X
			// The best position is not to move the camera at all
			if (camera.transform.position.x >= Min (camera).x && camera.transform.position.x <= Max (camera).x) pos.x = camera.transform.position.x;
			else if (character != null)
			{
				// If that doesn't work we want to try to place the camera exactly on the character
				if (character.transform.position.x >= Min(camera).x && character.transform.position.x <= Max(camera).x) pos.x = character.transform.position.x;
				// And failing that we find the closest boundary
			...
			// Y
			// The best position is not to move the camera at all
			if (camera.transform.position.y >= Min (camera).y && camera.transform.position.y <= Max (camera).y) pos.y = camera.transform.position.y;
			else if (character != null)
			{
				// If that doesn't work we want to try to place the camera exactly on the character
				if (character.transform.position.y >= Min(camera).y && character.transform.position.y <= Max(camera).y) pos.y = character.transform.position.y;
				// And failing that we find the closest boundary
		...
		void OnDrawGizmos()
		{
			...
			if (camera != null)
			{
				...
				if (camera.orthographic) {
					...
					UnityEditor.Handles.DrawSolidRectangleWithOutline(outerBounds , zoneColor, outlineColor);
				} else {
