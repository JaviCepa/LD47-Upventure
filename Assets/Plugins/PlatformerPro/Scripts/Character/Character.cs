...
namespace PlatformerPro
{
	...
	public class Character : PlatformerProMonoBehaviour, IMob
	{
		
#if UNITY_EDITOR
		override public string Header => "The base class for a playable character. This class ties together movement, animation, and other behaviours.";
		
		override public string DocLink => "https://jnamobile.zendesk.com/hc/en-gb/sections/200744754-Character-Components";
		...
		/// </summary>
		public bool snapToGroundOnSlopesOnly = true;
		...
		void Update()
		{
			if (Application.isPlaying && enabled && !TimeManager.Instance.Paused)
			{
		...
		/// Note: this method is used to set a characters facing direction during respawn and it is NOT about where the character spawns.
		/// If you want to respawn your character at a specific respawn point you should use LevelManager.Respawn(Character character, string respawnPoint)
		/// </summary>
		/// <param name="r">Respawn point to use for Facing Direction</param>
		/// <param name="facingDirection">Facing direction to set if the respawn point doesn't have a facing direction override.</param>
		virtual public void Respawn(RespawnPoint r, int facingDirection)
		{
			int originalFacingDirection = currentFacingDirection;
			if (r.facingDirection != 0) currentFacingDirection = r.facingDirection;
			else currentFacingDirection = facingDirection;
			// Check for facing direction changes
			if (switchCollidersOnDirectionChange)
			{
				if (originalFacingDirection != 0 && currentFacingDirection != 0 && currentFacingDirection != originalFacingDirection)
				{
					SwitchColliders();
				}
			}
					...
					if (CurrentCollisions [i] != null && 
						...
						for (int j = 0; j < CurrentCollisions [i].Length; j++) {
							if (CurrentCollisions [i] [j].collider != null && CurrentCollisions [i] [j].collider.gameObject != null &&
								...
								// If platform says ignore collissions then don't count it as a wall
								Platform tmpPlatform = CurrentCollisions[i][j].collider.GetComponent<Platform>();
								if (tmpPlatform == null || !tmpPlatform.IgnoreCollision(this, Colliders[i]))
								{
									if (detectAllWalls) {
										colliderCount++;
										if (tempWall == null) {
									...
									else
									{
										if (CurrentCollisions[i][j].collider.gameObject.layer == wallLayerNumber)
										{
											...
											// We cling to the highest wall using the highest collider
											else if (Colliders[currentWallCollider].WorldExtent.y <
											         Colliders[i].WorldExtent.y)
											{
												tempWall = CurrentCollisions[i][j].collider;
												currentWallCollider = i;
											}
						 	...
						 	(Colliders [i].RaycastType == RaycastType.SIDE_RIGHT && LastFacedDirection == -1)))
						{
							for (int j = 0; j < CurrentCollisions[i].Length; j++)
							{
								if (CurrentCollisions[i][j].collider != null &&
								    CurrentCollisions[i][j].collider.gameObject != null &&
								    // Only climb on geometry layers
								    (1 << CurrentCollisions[i][j].collider.gameObject.layer & geometryLayerMask) ==
								    1 << CurrentCollisions[i][j].collider.gameObject.layer)
								{
									Platform tmpPlatform = CurrentCollisions[i][j].collider.GetComponent<Platform>();
									if (tmpPlatform == null || !tmpPlatform.IgnoreCollision(this, Colliders[i]))
									{
										...
										{
											if (CurrentCollisions[i][j].collider.gameObject.layer ==
											    wallLayerNumber)
											{
												colliderCount++;
												if (tempWall == null)
