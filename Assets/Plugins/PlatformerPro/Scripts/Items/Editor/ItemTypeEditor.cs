...
namespace PlatformerPro 
{
	...
	public class ItemTypeEditor :  EditorWindow {
		...
		private const float lastColumnExtraWidth = 100.0f;
		...
		static void InitWindow() {
			float windowWidth = (columnWidth * 3.0f) + lastColumnExtraWidth;
		...
		void Init() {
			...
			itemDetailsHeaderLeftRect = new Rect (itemSelectRect.max.x + borderSize, itemSelectRect.min.y + 32.0f, columnWidth  - (borderSize * 2) , (2 * windowHeight / 3) - (borderSize + 32.0f) + 40);

			itemDetailsHeaderUpperRightRect = new Rect (itemDetailsHeaderLeftRect.max.x + (borderSize), itemSelectRect.min.y + 32.0f, columnWidth - (borderSize) + lastColumnExtraWidth , (windowHeight / 3) + 32.0f + 52);

			itemDetailsHeaderLowerRightRect = new Rect (itemDetailsHeaderLeftRect.max.x + (borderSize), itemDetailsHeaderUpperRightRect.max.y + borderSize, columnWidth - (borderSize) + lastColumnExtraWidth , (windowHeight / 3) - 76.0f - (2.0f * borderSize));

			itemDetailsRect = new Rect (itemSelectRect.max.x + borderSize, itemDetailsHeaderLeftRect.max.y + borderSize, columnWidth * 2.0f - (borderSize * 2) + lastColumnExtraWidth, windowHeight / 3 - (borderSize + 32.0f) - 8);
            ...
            {
                // Clean up paths
                manager.itemTypeDataLocation = manager.itemTypeDataLocation.Replace('/', Path.DirectorySeparatorChar);
                manager.itemTypeDataLocation = manager.itemTypeDataLocation.Replace('\\', Path.DirectorySeparatorChar);
                Load();
            }
			initialised = true;
		...
		void DrawItemHeaderLeft() {
			...
			GUILayout.Space (StandardVerticalSpacing);
			GUILayout.BeginHorizontal ();
			GUILayout.Label ("ID", EditorStyles.label, GUILayout.Width (labelWidth));
			...
			GUILayout.BeginHorizontal ();
			GUILayout.Label ("Name", EditorStyles.label, GUILayout.Width (labelWidth));
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label("Description", EditorStyles.label, GUILayout.Width(labelWidth));
            itemData[selectedItemIndex].description = EditorGUILayout.TextField(itemData[selectedItemIndex].description);
            GUILayout.EndHorizontal();

            GUILayout.Space(StandardVerticalSpacing);

            GUILayout.BeginHorizontal ();
			GUILayout.Label ("Level", EditorStyles.label, GUILayout.Width (labelWidth));
			...
			GUILayout.BeginHorizontal ();
			GUILayout.Label ("Price", EditorStyles.label, GUILayout.Width (labelWidth));
			...
			GUILayout.BeginHorizontal ();
			GUILayout.Label ("Stacking", EditorStyles.label, GUILayout.Width (labelWidth));
			...
			case ItemClass.NORMAL:
				...
				GUILayout.BeginHorizontal();
				GUILayout.Label ("Item Max", EditorStyles.label, GUILayout.Width (labelWidth));
				itemData [selectedItemIndex].itemMax = EditorGUILayout.IntField (itemData [selectedItemIndex].itemMax);
                GUILayout.EndHorizontal ();
				GUILayout.BeginHorizontal();
				GUILayout.Label ("Stack Max", EditorStyles.label, GUILayout.Width (labelWidth));
				if (itemData[selectedItemIndex].itemMax < 1) itemData[selectedItemIndex].itemMax = 1;
                itemData [selectedItemIndex].maxPerStack = EditorGUILayout.IntField (itemData [selectedItemIndex].maxPerStack);
				GUILayout.EndHorizontal ();
				GUILayout.BeginHorizontal();
				GUILayout.Label ("Starting Count", EditorStyles.label, GUILayout.Width (labelWidth));
				if (itemData[selectedItemIndex].maxPerStack < 1) itemData[selectedItemIndex].maxPerStack = 1;
                itemData [selectedItemIndex].startingCount = EditorGUILayout.IntField (itemData [selectedItemIndex].startingCount);
				GUILayout.EndHorizontal ();
				break;
			...
			GUILayout.BeginHorizontal ();
			GUILayout.Label ("Allow Drop", EditorStyles.label, GUILayout.Width (labelWidth));
			...
			{
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Drop Prefab", EditorStyles.label, GUILayout.Width (labelWidth));
		...
		void DrawItemBehaviour() {
			...
			GUILayout.BeginHorizontal ();
			GUILayout.Label ("Behaviour", EditorStyles.label, GUILayout.Width(labelWidth));
			...
			if (itemData[selectedItemIndex].itemBehaviour == ItemBehaviour.UPGRADE)
			{
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Do Upgrades Stack?", EditorStyles.label, GUILayout.Width(labelWidth));
				itemData[selectedItemIndex].upgradesStack = EditorGUILayout.Toggle(itemData[selectedItemIndex].upgradesStack);
				GUILayout.EndHorizontal ();
			}
            if (itemData[selectedItemIndex].itemClass == ItemClass.NORMAL && 
                itemData[selectedItemIndex].itemBehaviour == ItemBehaviour.CONSUMABLE)
            {
                GUILayout.Space(StandardVerticalSpacing);
                GUILayout.BeginHorizontal();
                GUILayout.Label("Clickable", EditorStyles.label, GUILayout.Width(labelWidth));
                itemData[selectedItemIndex].clickToConsume = EditorGUILayout.Toggle(itemData[selectedItemIndex].clickToConsume);
                GUILayout.EndHorizontal();
            }

            if (itemData[selectedItemIndex].itemClass == ItemClass.NORMAL &&
               itemData[selectedItemIndex].itemBehaviour == ItemBehaviour.EQUIPPABLE)
            {
                GUILayout.Space(StandardVerticalSpacing);
                GUILayout.BeginHorizontal();
                GUILayout.Label("Consumable", EditorStyles.label, GUILayout.Width(labelWidth));
                itemData[selectedItemIndex].consumableOnceEquipped = EditorGUILayout.Toggle(itemData[selectedItemIndex].consumableOnceEquipped);
                GUILayout.EndHorizontal();
            }
             ...
             itemData[selectedItemIndex].itemBehaviour == ItemBehaviour.EQUIPPABLE)
            {
                GUILayout.Space(StandardVerticalSpacing);
                GUILayout.BeginHorizontal();
                GUILayout.Label("Equip Multiple", EditorStyles.label, GUILayout.Width(labelWidth));
                itemData[selectedItemIndex].equipAll = EditorGUILayout.Toggle(itemData[selectedItemIndex].equipAll);
                GUILayout.EndHorizontal();
            }
			...
			{
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Slot", EditorStyles.label, GUILayout.Width (labelWidth));
				...
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Max Durability", EditorStyles.label, GUILayout.Width(labelWidth));
			...
			{
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Duration", EditorStyles.label, GUILayout.Width(labelWidth));
				...
				GUILayout.EndHorizontal ();
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Reset On Damage", EditorStyles.label, GUILayout.Width(labelWidth));
			...
			case "Score":
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Score Type", EditorStyles.label, GUILayout.Width (labelWidth));
				...
				{
					GUILayout.BeginHorizontal ();
					GUILayout.Label ("Score on Collect", EditorStyles.label, GUILayout.Width (labelWidth));
				...
				{
					GUILayout.BeginHorizontal ();
					GUILayout.Label ("Score on Consume", EditorStyles.label, GUILayout.Width (labelWidth));
					itemData [selectedItemIndex].scoreOnConsume = EditorGUILayout.IntField (itemData [selectedItemIndex].scoreOnConsume);
					GUILayout.EndHorizontal ();
				}
				break;
			case "Attack":
				// Only equippable weapons will use
				if (itemData[selectedItemIndex].itemBehaviour == ItemBehaviour.EQUIPPABLE)
				{
					GUILayout.BeginHorizontal();
					GUILayout.Label("Damage Type", EditorStyles.label, GUILayout.Width(labelWidth));
					itemData[selectedItemIndex].damageType = (DamageType) EditorGUILayout.EnumPopup(itemData[selectedItemIndex].damageType);
					GUILayout.EndHorizontal();
					if (itemData[selectedItemIndex].damageType != DamageType.NONE) EditorGUILayout.HelpBox("Damage Type only applies if the attacks weapon slot is also set to the slot value of this item", MessageType.Info);
				}
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Damage Multiplier", EditorStyles.label, GUILayout.Width (labelWidth));
				itemData [selectedItemIndex].damageMultiplier = EditorGUILayout.FloatField (itemData [selectedItemIndex].damageMultiplier);
				GUILayout.EndHorizontal ();
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Speed Multiplier", EditorStyles.label, GUILayout.Width (labelWidth));
			...
			case "Agility":
				...
				GUILayout.EndHorizontal ();
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Extra Jump Count", EditorStyles.label, GUILayout.Width (labelWidth));
			...
			case "Healing":
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("Max Health Adjustment", EditorStyles.label, GUILayout.Width (labelWidth));
				itemData [selectedItemIndex].maxHealthAdjustment = EditorGUILayout.IntField (itemData [selectedItemIndex].maxHealthAdjustment);
				GUILayout.EndHorizontal ();
				if (itemData[selectedItemIndex].itemBehaviour == ItemBehaviour.CONSUMABLE)
				{
					GUILayout.BeginHorizontal();
					GUILayout.Label("Health Adjustment", EditorStyles.label, GUILayout.Width(labelWidth));
					itemData[selectedItemIndex].healthAdjustment = EditorGUILayout.IntField(itemData[selectedItemIndex].healthAdjustment);
					GUILayout.EndHorizontal();
					GUILayout.BeginHorizontal();
					GUILayout.Label("Breath Adjustment", EditorStyles.label, GUILayout.Width(labelWidth));
		...
		void DrawSprites() {
			...
			Sprite icon = (Sprite) EditorGUILayout.ObjectField(itemData [selectedItemIndex].Icon, typeof (Sprite), false, GUILayout.Width(64), GUILayout.Height(64));
			...
			Sprite inGameSprite = (Sprite) EditorGUILayout.ObjectField(itemData [selectedItemIndex].InGameSprite, typeof (Sprite), false, GUILayout.Width(64), GUILayout.Height(64));
