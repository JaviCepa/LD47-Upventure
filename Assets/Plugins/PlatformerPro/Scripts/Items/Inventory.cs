...
namespace PlatformerPro
{
	...
	public class Inventory : PlatformerProMonoBehaviour, ICoreComponent 
	{
		...
		/// </summary>
		/// <param name="data">Item  instance data to add</param>
		public int AddItem(ItemInstanceData data)
		{
			ItemTypeData typeData = ItemTypeManager.Instance.GetTypeData(data.ItemId);
			if (typeData.maxPerStack != 1)
			{
				return AddStackableItem(typeData, data.amount);
			}
		...
		{
			ItemTypeData typeData = ItemTypeManager.Instance.GetTypeData(typeId);
			if (typeData.maxPerStack != 1)
			{
				return AddStackableItem(typeData, amount);
			}
		...
		/// <param name="amount">Amount.</param>
		protected int AddStackableItem(ItemTypeData type, int amount)
		{
            if (type.maxPerStack < 1)
            {
                Debug.LogWarning("Item configuration error, max per stack cannot be smaller than 1");
                return 0;
            }

            if (type.maxPerStack == 1)
            {
	            Debug.LogWarning("Tried to add item to a stack but it wasn't stackable'");
	            return 0; 
            }
	            
            if (amount > type.itemMax) amount = type.itemMax;
			int remainder = amount;
			// Check for existing stack of item and fill first - but items that can stack scant go in to stacks
		...
		/// </summary>
		public ItemInstanceData[] AllItems
		{
			get
			{
				return inventoryData.itemData;
			}
		}
